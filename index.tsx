      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-1">
        <form onSubmit={handleSubmit} className="relative">
          <textarea
            ...
            placeholder="Paste a sentence here... (e.g. 猫はベッドで寝ています)"
            className="w-full min-h-[120px] p-4 rounded-xl text-lg outline-none resize-none placeholder:text-slate-300 text-slate-700"
            disabled={isAnalyzing}
          />
          <div className="flex justify-between items-center px-2 pb-2">
            <span className="text-xs text-slate-400 font-semibold px-2">
              {input.length > 0 ? `${input.length} chars` : 'Ready to learn?'}
            </span>
            <button
              type="submit"
              ...
              className={`rounded-xl px-6 py-2.5 font-bold text-white flex items-center space-x-2 transition-all ${
                !input.trim() || isAnalyzing 
                  ...
                  : 'bg-emerald-500 hover:bg-emerald-600 shadow-md hover:shadow-lg transform hover:-translate-y-0.5'
              }`}
            >
              {isAnalyzing ? (
                <>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-white rounded-3xl shadow-xl border-b-4 border-slate-100 overflow-hidden">
            <div className="bg-slate-50 p-6 border-b border-slate-100">
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-slate-800 leading-relaxed mb-2">
                {lastResult.original}
              </h2>
                    ...
                    <div key={i} className="py-3 flex items-center justify-between group hover:bg-slate-50 transition-colors rounded-lg px-2 -mx-2">
                      <div>
                        <div className="flex items-baseline space-x-2">
                          ...
                          <span className="text-sm text-slate-400">{vocab.reading}</span>
                        </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.vocabulary.map((v, idx) => (
                        <div key={idx} className="text-xs bg-white border border-slate-200 px-2 py-1 rounded shadow-sm">
...
const DictionaryTab = ({ items }: { items: AnalysisResult[] }) => {
  ...
  // Aggregate all vocabulary
  const allVocab = items.flatMap(item => item.vocabulary).reduce((acc, curr) => {
    // Simple deduplication by word
    if (!acc.some(v => v.word === curr.word)) {
      acc.push(curr);
    }
    return acc;
  }, [] as VocabularyItem[]).sort((a, b) => a.word.localeCompare(b.word));
...
const App = () => {
  ...
  const handleAnalyze = async (text: string) => {
    ...
    try {
      ...
      // Stay on analyze tab to show result
    } catch (error) {
      console.error(error);
